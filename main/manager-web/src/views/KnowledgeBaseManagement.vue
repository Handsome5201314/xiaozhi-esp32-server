  ...
  methods: {
    ...
    showEditDialog: function(row) {
      ...
      this.knowledgeBaseForm = {
        ...
        permission: row.permission || "me",
        embeddingModel: row.embeddingModel || "",
        avatar: row.avatar || "",
        chunkMethod: row.chunkMethod || "naive",
        parseType: row.parseType !== undefined ? row.parseType : null,
        pipelineId: row.pipelineId || "",
        parserConfig: row.parserConfig || null
      };
      this.dialogVisible = true;
    },
    handleSubmit: function(form) {
      ...
      if (form.id) {
        ...
        const createData = {
          ...
          permission: form.permission || "me"
        };
        if (form.embeddingModel) {
          createData.embeddingModel = form.embeddingModel;
        }
        if (form.avatar) {
          createData.avatar = form.avatar;
        }
        if (form.pipelineId) {
          createData.parseType = form.parseType;
          createData.pipelineId = form.pipelineId;
        } else {
          createData.chunkMethod = form.chunkMethod || "naive";
        }
        if (form.parserConfig) {
          createData.parserConfig = form.parserConfig;
        }
