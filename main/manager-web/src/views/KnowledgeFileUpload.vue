<template>
  <div class="welcome">
      ...
      <div class="content-panel">
        <div class="content-area">
          <el-card class="params-card" shadow="never">
            <div class="inheritance-banner" v-if="inheritanceSummary">
              <span>{{ $t('knowledgeFileUpload.inheritParserConfig') }}</span>
              <strong>{{ inheritanceSummary }}</strong>
            </div>
    ...
    <el-dialog :title="$t('knowledgeFileUpload.uploadDocument')" :visible.sync="uploadDialogVisible" width="800px">
      <div class="inheritance-banner upload-inheritance-banner" v-if="inheritanceSummary">
        <span>{{ $t('knowledgeFileUpload.inheritParserConfig') }}</span>
        <strong>{{ inheritanceSummary }}</strong>
      </div>
...
export default {
  ...
  computed: {
    ...
    inheritanceSummary() {
      if (!this.knowledgeBaseConfig) {
        return '';
      }
      if (this.knowledgeBaseConfig.pipelineId) {
        return `Pipeline ${this.knowledgeBaseConfig.pipelineId} / parse type ${this.knowledgeBaseConfig.parseType}`;
      }
      const parserConfig = this.parseExistingParserConfig(this.knowledgeBaseConfig.parserConfig);
      const method = this.getChunkMethodLabel(this.knowledgeBaseConfig.chunkMethod || 'naive');
      const layout = parserConfig.layout_recognize || 'RAGFlow 默认解析器';
      const chunk = parserConfig.chunk_token_num ? `chunk ${parserConfig.chunk_token_num}` : '默认切片';
      const extras = [];
      if (parserConfig.auto_keywords !== undefined) {
        extras.push(`keywords ${parserConfig.auto_keywords}`);
      }
      if (parserConfig.auto_questions !== undefined) {
        extras.push(`questions ${parserConfig.auto_questions}`);
      }
      return [method, layout, chunk, ...extras].join(' / ');
    },
  },
  methods: {
    goBack() {
    ...
    fetchKnowledgeBaseDetail: function () {
      if (!this.datasetId) {
        return;
      }
      KnowledgeBaseAPI.getKnowledgeBaseDetail(this.datasetId,
        ({ data }) => {
          if (data && data.code === 0) {
            this.knowledgeBaseConfig = data.data || null;
          }
        },
        (err) => {
          console.warn('获取知识库解析配置失败:', err);
        }
      );
    },
    parseExistingParserConfig: function (parserConfig) {
      if (!parserConfig) {
        return {};
      }
    ...
    getChunkMethodLabel: function (value) {
      const labels = {
        naive: this.$t('knowledgeBaseDialog.chunkMethodNaive'),
        paper: this.$t('knowledgeBaseDialog.chunkMethodPaper'),
        table: this.$t('knowledgeBaseDialog.chunkMethodTable'),
        qa: this.$t('knowledgeBaseDialog.chunkMethodQa'),
        book: this.$t('knowledgeBaseDialog.chunkMethodBook'),
        laws: this.$t('knowledgeBaseDialog.chunkMethodLaws'),
        presentation: this.$t('knowledgeBaseDialog.chunkMethodPresentation'),
        picture: this.$t('knowledgeBaseDialog.chunkMethodPicture'),
        one: this.$t('knowledgeBaseDialog.chunkMethodOne'),
        manual: this.$t('knowledgeBaseDialog.chunkMethodManual'),
        knowledge_graph: this.$t('knowledgeBaseDialog.chunkMethodKnowledgeGraph'),
        email: this.$t('knowledgeBaseDialog.chunkMethodEmail')
      };
